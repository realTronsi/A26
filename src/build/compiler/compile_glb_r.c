...
SyntaxTree_* rodata_glb_reg(SyntaxTree_* tree, int p_reg_index,FILE* file,SyntaxTree_* all)
{
  for(int i = 0; i < tree->p_references_indexes->list_len; i++)
  {
    ...
    {
      for(int i = 0; i < all->trees[p_reg_index]->p_reg_values->list_len; i++)
      {
        char* val = all->trees[p_reg_index]->p_reg_values->values[i];
        int minus = 0;
        for(int x = 0; x < strlen(val); x++)
        {
          if(val[x] == '\\') minus++;
        }
        fprintf(file,"\n\tmov ecx, p_msg_%d\n\tmov edx, %ld\n\tmov eax, 4\n\tmov ebx, 1\n\tsys_call\n",i+1,strlen(all->trees[p_reg_index]->p_reg_values->values[i])-minus);
      }
    }
    else
    {
